...
def _extract_table_data(page_html: str) -> list[dict[str, str | None]]:
    ...
    for row in table.find_all("tr")[1:]:  # Skip header row.
        ...
        for idx, cell in enumerate(row.find_all("td")):
            ...
            if column_name == "Hardware":
                # Hardware often contains several links.
                for link in cell.find_all("a"):
                    if link and ("href" in link.attrs):
                        hardware_link_text = link.get_text(strip=True)
                        hardware_count, hardware_type = hardware_link_text.split(
                            " ", maxsplit=1
                        )

                        # Add columns like "gpu_count" and "gpu_href".
                        row_data[f"{hardware_type.lower()}_count"] = hardware_count
                        row_data[f"{hardware_type.lower()}_href"] = (
...
def main() -> None:
    ...
    for page_num in range(1, 5):  # Only 3 pages right now.
        logger.info(f"Fetching page {page_num}")
        url = f"https://poolbay.io/algorithms?page={page_num}"
    ...
    df.write_parquet(step_1_output_folder / "poolbay_algorithms.parquet")
    df.write_csv(step_1_output_folder / "poolbay_algorithms.csv")
